...
/**
 * Parse all where options
 * @param {string} where A single where string or an array of where strings
 *     in the format prop.comparator=value
 * @param {Object} query The query object.
 */
function parseWhere(where, query) {

  function parseClause(clause) {
    var parts = clause.split('=');
    if (parts.length !== 2) {
      log.warn('Ignoring malformed where clause: ', clause);
    } else {
...
var options = {
  ...
  where: {
    description: 'Filter by any feature property (eg. sat.alt.gt=300)',
    type: 'string'
  }
};
