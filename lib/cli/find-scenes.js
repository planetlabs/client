...
function parseAcquired(acquired, query) {
  ...
  if (parts.length === 1) {
    if (isNaN(start.getTime())) {
      throw new Error('Invalid date for "acquired" option: ' + acquired);
    }
    ...
    if (parts[0]) {
      if (isNaN(start.getTime())) {
        throw new Error('Invalid start date for "acquired" option: ' + acquired);
      }
      query['acquired.gte'] = start.toISOString();
    }
    if (parts[1]) {
      if (isNaN(end.getTime())) {
        throw new Error('Invalid end date for "acquired" option: ' + acquired);
      }
      query['acquired.lt'] = end.toISOString();
    }
  } else {
    throw new Error('Invalid range for the "acquired" option; ' + acquired);
  }
}
